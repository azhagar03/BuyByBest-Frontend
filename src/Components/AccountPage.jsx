...
const AccountPage = ({ user, onNavigate, onLogout }) => {
  ...
  const fetchOrders = async () => {
    ...
    try {
      ...
      const res  = await fetch(`https://buybybest-backend-0khu.onrender.com/orders/user/${uid}`);
  ...
  const handleCancel = async (orderId) => {
    ...
    try {
      const res  = await fetch(`https://buybybest-backend-0khu.onrender.com/orders/${orderId}/cancel`, { method: "PUT" });
      const data = await res.json();
      if (data.success) {
